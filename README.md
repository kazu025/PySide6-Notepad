...
テキストの折り返し表示（Word Wrap）のON / OFF機能を追加しました。

表示メニューから折り返し表示を切り替えることができ、
折り返しをOFFにした場合は長い行を横スクロールで表示できます。

また、折り返し表示の設定をQSettingsに保存し、
アプリを再起動しても前回の設定を保持します。

### 追加した機能

* 折り返し表示のON / OFF
* 表示メニューの追加
* checkableなQActionによる状態切り替え
* `QTextEdit.LineWrapMode.WidgetWidth` による折り返し表示
* `QTextEdit.LineWrapMode.NoWrap` による折り返し解除
...
![Main Window](images/mainwindow_ver10.0.png)
